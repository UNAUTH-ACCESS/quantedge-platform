 ...
 *
 * ⚠️  PUBLIC REPO NOTICE: the simulation calibration constants below
 * (slippage/rejection-rate/delay) are illustrative placeholders in this
 * public copy — the tuned values used in the real simulation layer are not
 * shown. The orchestration logic (proposal creation, fill recording, P&L
 * calculation, settlement handoff) is real and unmodified.
 */
...
// [REDACTED] Illustrative placeholders — real calibration values not shown.
const SLIPPAGE_NORMAL = 0.001;
const SLIPPAGE_STRESS = 0.003;
const REJECTION_RATE  = 0.05;
...
async function closePosition(positionId, reason = "MANUAL") {
  try {
    ...
    // Settle on-chain — return realized amount to user wallet. settlePosition
    // handles its own retries/backoff and marks SETTLEMENT_FAILED + alerts on
    // exhaustion. We await it so the position record always reflects the true
    // on-chain settlement outcome before this function returns.
    try {
